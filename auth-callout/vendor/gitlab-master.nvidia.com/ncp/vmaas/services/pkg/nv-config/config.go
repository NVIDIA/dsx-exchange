...
import (
	...
	"gitlab-master.nvidia.com/ncp/vmaas/services/pkg/nv-config/internal/providers"
)
...
// WithConfigStruct registers a config struct to extract descriptions from desc tags and default values
...
func NewService(defaultYAML string, serviceConfig interface{}, envPrefix string, providers []LibraryConfigProvider, cobraCmd *cobra.Command) (*ConfigManager, error) {
	...
	if cobraCmd != nil {
		...
		if !hasCmd {
			root.AddCommand(newGenerateConfigCmd(cm))
		}
	}
...
func (cm *ConfigManager) Load() error {
	...
	// 2. Load default YAML and struct if provided
	if cm.defaultStruct != nil {
		if err := cm.loadDefaultStruct(); err != nil {
			return err
		}
	}
...
func newGenerateConfigCmd(cm *ConfigManager) *cobra.Command {
	return &cobra.Command{
		Use:   "generate-config",
		Short: "Print the complete default configuration as YAML",
		RunE: func(cmd *cobra.Command, args []string) error {
			if err := cm.Load(); err != nil {
				return fmt.Errorf("failed to load config: %w", err)
			}
			yamlBytes, err := cm.ExportYAML()
			if err != nil {
...
func (cm *ConfigManager) loadDefaultStruct() error {
	if err := cm.koanf.Load(structs.Provider(cm.defaultStruct, "koanf"), nil); err != nil {
		return &ConfigError{
			Type:    ErrorTypeLoad,
			Message: "failed to load default struct",
			Source:  "default_struct",
			Cause:   err,
		}
	}
	cm.metadata.Sources["default_struct"] = "embedded_struct"
	return nil
}
...
func (cm *ConfigManager) generateCobraFlags() error {
	// Infrastructure flags (--config, --hot-config, --hot-config-interval) are registered
	// as PersistentFlags so they propagate to subcommands. This allows services with
	// multiple subcommands (e.g., server + sidecar) to share a single ConfigManager.
	if cm.configFlag != "" && cm.cobraCmd.PersistentFlags().Lookup(cm.configFlag) == nil {
		defaultValue := strings.Join(cm.standardConfigFiles, ",")
		cm.cobraCmd.PersistentFlags().String(cm.configFlag, defaultValue,
			"Comma-separated list of YAML configuration files to load")
	}

	if cm.hotConfigFlag != "" && cm.cobraCmd.PersistentFlags().Lookup(cm.hotConfigFlag) == nil {
		...
		cm.cobraCmd.PersistentFlags().String(cm.hotConfigFlag, defaultValue, helpText)
	}

	intervalFlagName := cm.hotConfigFlag + "-interval"
	if cm.hotConfigFlag != "" && cm.cobraCmd.PersistentFlags().Lookup(intervalFlagName) == nil {
		...
		cm.cobraCmd.PersistentFlags().Duration(intervalFlagName, cm.reloadInterval, helpText)
	}
...
func (cm *ConfigManager) loadInitialDefaults() error {
	...
	// 2. Load default struct if provided
	if cm.defaultStruct != nil {
		if err := cm.loadDefaultStruct(); err != nil {
			return err
		}
	...
	// 3. Load default YAML if provided
	if cm.defaultYAML != "" {
		if err := cm.loadDefaultYAML(); err != nil {
			return err
		}
...
// Example: "cache.backends.{key}.host" → "MYAPP_CACHE_BACKENDS_{KEY}_HOST"

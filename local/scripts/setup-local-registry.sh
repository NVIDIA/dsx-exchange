...
for command in curl docker jq kind; do
...
write_registry_config() {
  local mirror="${DOCKERHUB_MIRROR:-}"

  if [[ -z "${mirror}" ]]; then
    cp "${REGISTRY_CONFIG}" "${runtime_config}"
    return
  fi
  ...
  jq --arg mirror "${mirror}" '
    (.extensions.sync.registries[] |
      select(any(.content[]?; .destination == "/docker.io")) |
      .urls) |= ([$mirror] + map(select(. != $mirror)))
  ' "${REGISTRY_CONFIG}" > "${runtime_config}"
}
...
config_hash="$(hash_file "${runtime_config}")"
...
if ! docker inspect "${REGISTRY_NAME}" >/dev/null 2>&1; then
  docker create \
    ...
    "${image}" >/dev/null
  docker cp "${runtime_config}" "${REGISTRY_NAME}:/etc/zot/config.json"
fi
...
done < <(kind get nodes --name dsx-exchange)

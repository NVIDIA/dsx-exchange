...
# Run every performance/*.js scenario as a k6 process on the host against the
...
#
# Outputs land in reports/<scenario><suffix>.{summary.json,
# ndjson}. k6 exits 99 on threshold breach.
#
# Usage:
#   bash local/agent-gateway/run-perf.sh <gateway-url-without-/mcp>
set -euo pipefail
...
ARTIFACTS="${ARTIFACTS:-$TEST_ROOT/reports}"
FUNCTIONAL="$TEST_ROOT/tests/functional"
PERFORMANCE="$TEST_ROOT/tests/performance"
...
RUN_PERF_PROFILE="${RUN_PERF_PROFILE:-smoke}"
...
TOKEN_A="$(cd "$FUNCTIONAL" && go run ./cmd/mint-token tenant-a)"
TOKEN_B="$(cd "$FUNCTIONAL" && go run ./cmd/mint-token tenant-b)"
...
# both scenarios' windows, proving load-shed behavior concurrently.
echo "==> k6 scenarios in parallel"
for p in "$PERFORMANCE"/*.js; do
  run_k6_scenario "$p" &
  perf_pids+=("$!")
  perf_names+=("$p")
done

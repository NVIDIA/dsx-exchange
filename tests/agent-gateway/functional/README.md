...
- `tests/agent-gateway/run.sh` runs `go test -json` and fails on
...
`tests/agent-gateway/run.sh` exports `GATEWAY_URL`, defaulting to
`http://localhost:18180/mcp` for kind. Direct `go test` runs must set it. No

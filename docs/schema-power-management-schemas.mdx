...
| `load_constraint` | [LoadConstraint](#loadconstraint) | Yes | Power limit constraint. |
...
| `interval` | [Interval](#interval) | Yes | Time window for load target scheduling. |
...
| `strategy` | [Strategy](#strategy) | Yes | Load shedding strategy configuration. |
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `metadata` | [FeedMetadata](#feedmetadata) | Yes | Static feed configuration. Values don't change during a curtailment event. |
...
| `targets` | array\<[FeedTarget](#feedtarget)\> | Yes | All load targets for this feed — active and scheduled. Empty array if no targets exist. |
...
| `feeds` | map\&lt;string, [FeedState](#feedstate)\> | Yes | Map of feed_tag to power state. |
...
| `breach` | [BreachDetails](#breachdetails) | Yes | Breach identification and timing. |
...
| `target` | [BreachTarget](#breachtarget) | Yes | The load target being breached. |
| `target.correlation_id` | string (uuid) | Yes | The correlationid from the grid.loadtarget.set.v1 that established this target. |
| `target.load_constraint` | [LoadConstraint](#loadconstraint) | Yes | Power limit constraint. |
...
| `shed_hints` | array\<[ShedHint](#shedhint)\> | No | Ordered list of load shedding recommendations. Present when status is active or escalated. Absent when resolved. |
...
| `results` | array\<[EnforcementResult](#enforcementresult)\> | Yes | Per-resource enforcement outcome. |
...
| `traceparent` | string | Yes | REQUIRED per W3C Trace Context and CloudEvents Distributed Tracing Extension. Format: version-traceid-parentid-flags. |
| `tracestate` | string | No | OPTIONAL per W3C Trace Context. Carries vendor-specific contextual information across tracing systems. |
| `data` | [JwsCompactSerialization](#jwscompactserialization) | [SetLoadTargetData](#setloadtargetdata) | Yes |
...
| `traceparent` | string | Yes | REQUIRED per W3C Trace Context and CloudEvents Distributed Tracing Extension. Format: version-traceid-parentid-flags. |
| `tracestate` | string | No | OPTIONAL per W3C Trace Context. Carries vendor-specific contextual information across tracing systems. |
| `data` | [JwsCompactSerialization](#jwscompactserialization) | [PowerStateStatusData](#powerstatestatusdata) | Yes |
...
| `traceparent` | string | Yes | REQUIRED per W3C Trace Context and CloudEvents Distributed Tracing Extension. Format: version-traceid-parentid-flags. |
| `tracestate` | string | No | OPTIONAL per W3C Trace Context. Carries vendor-specific contextual information across tracing systems. |
| `data` | [JwsCompactSerialization](#jwscompactserialization) | [PowerBreachAlertData](#powerbreachalertdata) | Yes |
...
| `traceparent` | string | Yes | REQUIRED per W3C Trace Context and CloudEvents Distributed Tracing Extension. Format: version-traceid-parentid-flags. |
| `tracestate` | string | No | OPTIONAL per W3C Trace Context. Carries vendor-specific contextual information across tracing systems. |
| `data` | [JwsCompactSerialization](#jwscompactserialization) | [PowerBreachEnforcementData](#powerbreachenforcementdata) | Yes |

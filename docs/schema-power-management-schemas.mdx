...
| `load_constraint` | [LoadConstraint](schema-power-management-schemas#loadconstraint) | Yes | Power limit constraint. |
...
| `interval` | [Interval](schema-power-management-schemas#interval) | Yes | Time window for load target scheduling. |
...
| `strategy` | [Strategy](schema-power-management-schemas#strategy) | Yes | Load shedding strategy configuration. |
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `metadata` | [FeedMetadata](schema-power-management-schemas#feedmetadata) | Yes | Static feed configuration. Values don't change during a curtailment event. |
...
| `targets` | array\<[FeedTarget](schema-power-management-schemas#feedtarget)\> | Yes | All load targets for this feed — active and scheduled. Empty array if no targets exist. |
...
| `feeds` | map\&lt;string, [FeedState](schema-power-management-schemas#feedstate)\> | Yes | Map of feed_tag to power state. |
...
| `breach` | [BreachDetails](schema-power-management-schemas#breachdetails) | Yes | Breach identification and timing. |
...
| `target` | [BreachTarget](schema-power-management-schemas#breachtarget) | Yes | The load target being breached. |
| `target.correlation_id` | string (uuid) | Yes | The correlationid from the grid.loadtarget.set.v1 that established this target. |
| `target.load_constraint` | [LoadConstraint](schema-power-management-schemas#loadconstraint) | Yes | Power limit constraint. |
...
| `shed_hints` | array\<[ShedHint](schema-power-management-schemas#shedhint)\> | No | Ordered list of load shedding recommendations. Present when status is active or escalated. Absent when resolved. |
...
| `results` | array\<[EnforcementResult](schema-power-management-schemas#enforcementresult)\> | Yes | Per-resource enforcement outcome. |
...
| `traceparent` | string | Yes | REQUIRED per W3C Trace Context and CloudEvents Distributed Tracing Extension. Format: version-traceid-parentid-flags. |
| `tracestate` | string | No | OPTIONAL per W3C Trace Context. Carries vendor-specific contextual information across tracing systems. |
| `data` | [JwsCompactSerialization](schema-power-management-schemas#jwscompactserialization) or [PowerStateStatusData](schema-power-management-schemas#powerstatestatusdata) | Yes | WIRE FORMAT: JWS compact serialization string (type: string). PLAINTEXT CONTENT: After JWS verification, the payload conforms to PowerStateStatusData schema (type: object). |
...
| `traceparent` | string | Yes | REQUIRED per W3C Trace Context and CloudEvents Distributed Tracing Extension. Format: version-traceid-parentid-flags. |
| `tracestate` | string | No | OPTIONAL per W3C Trace Context. Carries vendor-specific contextual information across tracing systems. |
| `data` | [JwsCompactSerialization](schema-power-management-schemas#jwscompactserialization) or [PowerBreachAlertData](schema-power-management-schemas#powerbreachalertdata) | Yes | WIRE FORMAT: JWS compact serialization string (type: string). PLAINTEXT CONTENT: After JWS verification, the payload conforms to PowerBreachAlertData schema (type: object). |
...
| `traceparent` | string | Yes | REQUIRED per W3C Trace Context and CloudEvents Distributed Tracing Extension. Format: version-traceid-parentid-flags. |
| `tracestate` | string | No | OPTIONAL per W3C Trace Context. Carries vendor-specific contextual information across tracing systems. |
| `data` | [JwsCompactSerialization](schema-power-management-schemas#jwscompactserialization) or [PowerBreachEnforcementData](schema-power-management-schemas#powerbreachenforcementdata) | Yes | WIRE FORMAT: JWS compact serialization string (type: string). PLAINTEXT CONTENT: After JWS verification, the payload conforms to PowerBreachEnforcementData schema (type: object). |
